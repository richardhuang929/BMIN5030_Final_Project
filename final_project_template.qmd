...
dat <- dat %>%
  mutate(
    ...
    adl_lim = factor(adl, levels = c(0, 1), labels = c("No", "Yes")),
    ...
    # Depression: 1 = yes
    dep_cat = factor(depression, levels = c(0, 1), labels = c("No", "Yes")),
...
```{r}
baseline_vars <- c(
  "age", "sex", "edu_cat", "marital_cat", "work_cat",
  "smoke_cat", "alcohol_cat", "exercise_cat",
  "bmi", "adl_lim", "dep_cat", "cd_cat", "HAQ", "GDP"
)
...
suffix_m1 <- ""
suffix_m2 <- "+ age + sex + edu_cat + work_cat + marital_cat"
suffix_m3 <- paste(
  suffix_m2,
  "+ bmi + smoke_cat + alcohol_cat + exercise_cat +",
  "adl_lim + cd_cat + dep_cat"
)
suffix_m4 <- paste(
  suffix_m3,
  "+ HAQ + GDP"
    ...
    left_join(
      dat %>%
        select(
          ...
          age, sex, edu_cat, marital_cat, work_cat,
          bmi, smoke_cat, alcohol_cat, exercise_cat,
          adl_lim, cd_cat, dep_cat, HAQ, GDP
        ),
      by = "mergeid"
    ) %>%
...
base_adjust_vars <- c(
  "age", "sex", "edu_cat", "work_cat", "marital_cat",
  "bmi", "smoke_cat", "alcohol_cat", "exercise_cat",
  "adl_lim", "cd_cat", "dep_cat",
  "HAQ", "GDP"
)
...
sub_sex <- run_subgroup_with_interaction(
  ...
  subgroup_var = "sex",
  subgroup_label = "Sex",
  adjust_vars  = setdiff(base_adjust_vars, "sex")
)
...
sub_edu <- run_subgroup_with_interaction(
  data         = dat_6_8,
  subgroup_var = "edu_cat",
  subgroup_label = "Education",
  adjust_vars  = setdiff(base_adjust_vars, "edu_cat")
)
...
ggplot(subgroup_results_plot,
  ...
  labs(
    ...
    y = "",
    title = "Subgroup analysis of TG/HDL-C and 4-year cognitive impairment (Wave 6–Wave 8)"
  ) +
...
No strong evidence of effect modification was detected.\
Specifically:

-    **Age (\<75 vs ≥75):** Associations remained positive in both strata, with *p*-for-interaction values indicating no significant interaction.

-    **Sex:** Both men and women exhibited similar patterns of increased risk with higher TG/HDL-C.

-    **BMI categories (\<25, 25–29.9, ≥30):** Associations were similar across weight groups.

-    **Education:** Associations persisted across education levels without significant subgroup differences.
